...
| Full validation (L2–L5) | manual on reference machine (`package.py --model E:\glm-5.2-bf16`) | before tag |
| Packaging + release | `ci.yml` jobs `build-and-gates` (CPack) + `release` (tag `v*`) | **only after Build + L0 + L1 + L6 pass 100% on all three OS; then auto-publishes a GitHub Release with the archives and gate reports** |
